...
class Users extends MX_Controller {
	...
	{		
		...
		else
		{
			...
			else
			{
				...
				// // Print the raw hashed password and the encrypted result.
				// echo "Raw Password: " . $password . "<br>";
				// echo "Hashed Password (pre-encryption): " . hash('sha256', $password) . "<br>";
				...
				if($doc_name)
				{
			        ...
			        if($userid && $users_roles_id && $users_documents_id)
			        {
						...
						// // var_dump($data[0]);
						// // die;
						// // $this->sendmail->sendmailto($email,'Verify email address',$msg);

						// if($this->sendmail->sendmailto($email,'Verify email address',$msg))
						// {
						// 	$arg['status']  = 1;
						...
						$arg['error_code']   = SUCCESS_CODE;
						$arg['error_line']= __line__;
						$arg['message'] = "Registration Succesful";
						//$arg['url']     = $url;
						$arg['user_id']  = "$userid";
						$arg['data']['user_id']  = "$userid";
			        }
	...
	public function login()
	{
			...
			if($_POST)
			{
				...
				else
				{
					...
					if($data)
					{
						...
						{
							$varify = 1;
						}elseif(isset($social_auth_id) && $social_auth_id== $data[0]['social_auth_id']){
		...
		// $receiver = $this->dynamic_model->get_user_by_id(226);
		// log_message('debug', 'device_token - data: ' . $receiver['Device_Token']);
		// $ref_where = array('User_Id' => 226);	
		// $get_referrals_data = $this->dynamic_model->getdatafromtable('user_in_roles',$ref_where,'Device_Id,Device_Type');
		// log_message('debug', 'device_token - data: ' . $get_referrals_data['Device_Id']. ' &&& Device_Type = '. $get_referrals_data['Device_Type']);
	    echo json_encode($arg);
	}
					...
					if(!empty($data))
					{
						if($data[0]['Is_Email_Verified']==1){
							...
							if(empty($otp_user))
							{
								...
								$emaildata['subject']     = $template_data[0]['subject']; 
								$emaildata['description'] = $desc;  
								$emaildata['body']        = 'this is testing email';
	...
	function testEmail(){
		...
		$message = "The Following staff has requested for leave details are mentioned below: <br>";
		$message .= " Link : https://itsths.com <br>";
	  
		$this->sendmail->send_email_phpMailer($to = "ateam.dev99@gmail.com", $subject, $message, null, null, 'ateam.dev99@gmail.com');
				...
				else
				{
					...
					if($loguser)
					{
						...
						if(!empty($advertisement_data))
						{
							...
							// $account_data = $this->dynamic_model->getdatafromtable('accounts', array('user_id'=>$usid));
							// $account_data = $this->dynamic_model->get_row('accounts',array('user_id'=> $usid));

							// $conditionn=array('receiver_id'=>$loguser['Id']);
							$userid= $loguser['Id'];
							// $transaction_data=array();
							// $transaction_data = $this->dynamic_model->getdatafromtable_new('hedera_transactions', $conditionn , '*', $limit=5, '','Id','');
							...
							if(!empty($transaction_data))
							{
								...
								foreach($transaction_data as $details)
			            		{
			            			...
			            			// 	$Requested_user = $this->dynamic_model->get_user_by_id($details['sender_id']);

									if($details['sender_id']){
			            				$Requested_user = $this->dynamic_model->get_user_by_id($details['sender_id']);
									}
			            			if($details['receiver_id']){
			            				// $To_user = $this->dynamic_model->get_user_by_id($details['receiver_id']);
										
										$To_user = $this->dynamic_model->get_receiver_by_id($details['receiver_id']);
									}
			            			...
			            			// {
			            				...
			            				if($Requested_user['FullName'] != "")
			            				{
			            					$user_data["title"]          = $details['sender_id'] == $userid ? "Sent " : "Received ";
			            				}
			            				$user_data["fullname"]           = $details['sender_id'] == $userid ? $To_user['FullName'] : $Requested_user['FullName'];
			            				$user_data["user_image"]         = site_url().'uploads/user/'.$Requested_user['Profile_Pic'];
			            				$user_data["mobile"]             = "";
			            				$user_data["order_number"]       = $details['transaction_id'];
			            				$user_data["amount"]             = number_format($details['amount'],2);
			            				$user_data["charge"]             = number_format($details['Charge'],2);
			            				$user_data['created_at']         = date('d M Y', strtotime($details['created_at']));
			            				if($transaction_status['Status_Name'] == "Reject")
			            					$user_data['transaction_status'] = "Failed";
			            				else
			            					$user_data['transaction_status'] = $details['status'];
			            				$user_data['trx_type']           = "Money Received";
			            				// $user_data['tran_type_id']       = $details['Tran_Type_Id'];
			            				// $user_data['tran_status_id']      = $details['Tran_Status_Id'];
			            				$user_data['time']               = date('g:i A',strtotime($details['created_at']));
			            				$user_data['msg']                = (!empty($details['Msg'])) ? $details['Msg'] : "";
			            				$user_data['receiver']                = $To_user;
			            				$user_data['organization_title']  = $org_name;
			            				$transaction_array[]             = $user_data;
			            			// }	
			            		}	
						...
						if($loguser['private_key'] && $loguser['account_id']){
							$isPrivateKeyAvailable = true;
						}
						$arg['account'] = $account_data;
						$arg['status'] = 1;
						$arg['error_code']   = SUCCESS_CODE;
			            $arg['error_line']= __line__;
					    ...
					    $arg['data']['is_private_key_available']         = $isPrivateKeyAvailable;
					    ...
					    $arg['data']['token_balance']    	= number_format($loguser['hbar_balance'],2);
						$arg['data']['token_value']    		= number_format($loguser['token_value']);
						// $arg['data']['hbar_balance']    	= number_format($loguser['hbar_balance'],2);
						$arg['data']['hbar_balance']    	= number_format($loguser['hbar_balance'],2);
...
public function get_profile()
{
        ...
        if ($version_result['status'] != 1) {
            ...
            if ($result != 'true') {
                ...
                if ($loguser) {
					...
					$id_base64 = base64_encode($loguser['Id']);
                    $arg['status'] = 1;
                    $arg['error_code']   = SUCCESS_CODE;
                    $arg['error_line']   = __LINE__;
                    $arg['data']['user_id']                = $loguser['Id'];
                    $arg['data']['base64_id']                = $id_base64;
				...
				else
				{
					...
					if($bank_detail)
					{
						...
						foreach ($bank_detail as $bank)
						{
							...
							$user_data["acc_number"]  = decode_id($bank['Account_No']);
							$user_data["acc_holder_name"] = $bank['Acc_Holder_Name'];
							$user_data["branch_name"] = $bank['Branch_Name'];
							$user_data["account_holder_type"] = $bank['account_holder_type'];
							$user_data["routing_number"] 	= $bank['routing_number'];
							$user_data["phone_number"]		= $bank['phone_number'];
							//$user_data["bank_image"]  = $imagePath;
	...
	// 							// $transaction_data = $this->dynamic_model->getdatafromtable_new('hedera_transactions', $search_text , '*', $limit, $offset,'Id',$search_false);
	...
	// 							// $transaction_data = $this->dynamic_model->getdatafromtable('hedera_transactions', array('receiver_id'=>$userid) , '*', $limit, $offset,'Id');
	// 						}
	// 						$transaction_array=array();
	// 						if(!empty($transaction_data))
	// 						{
	...
	// 		            				$To_user = $this->dynamic_model->get_user_by_id($details['To_User_Id']);
	// 								}
	// 		            			if(!empty($details['organisation_id'])){
	// 		            				$org_data = $this->dynamic_model->getdatafromtable('organization_details',array('Id'=>$details['organisation_id']),'Organization_Name');
	// 		            				$org_name=(!empty($org_data[0]['Organization_Name'])) ? $org_data[0]['Organization_Name'] : "";
	...
	// 		            				{
	// 		            					$user_data["title"]              = "Paid to";
	// 		            					$user_data["sender_fullname"]    = $Requested_user['FullName'];
	// 		            					$user_data["receiver_fullname"]  = $To_user['FullName'];
	// 		            					$user_data["user_image"]         = site_url().'uploads/user/'.$Requested_user['Profile_Pic'];
	// 		            					$user_data["mobile"]             = $Requested_user['Mobile_No'];
	// 		            				}
	// 		            				$user_data["order_number"]       = $details['Third_Party_Tran_Id'];
				...
				else
				{
					...
					if($_POST)
					{
						...
						else
						{
							...
							$filter_type = $this->input->post('filter_type');
							$search_text = array();
							$search_text["receiver_id"] = $userid ;
							if(!empty($filter_type)){
								if($filter_type == '1'){ // this month
									$search_text["MONTH(Creation_Date_Time)"] = "MONTH(CURRENT_DATE())"; 
									$search_false = false;
    ...
    public function get_about_us()
    {
		...
		else
		{
			...
			$arg['error_line']= __line__;
			$arg['message'] = "";
	        $arg['data']    = array('url' => site_url().'about.html');
	    }
		echo json_encode($arg);
	}
    ...
    public function get_privacy_policy()
    {
		...
		else
		{
			...
			$arg['error_line']= __line__;
			$arg['message'] = "";
	        $arg['data']    = array('url' => site_url().'privacy_policy.html');
	    }
        echo json_encode($arg);
    }
    ...
    public function get_terms_condition()
    {
	   $arg = array();
		...
		else
		{
			...
			$arg['error_line']= __line__;
			$arg['message'] = "";
	        $arg['data']    = array('url' => site_url().'terms_and_conditions.html');   
	    }
	    echo json_encode($arg);
    } 
				...
				else
				{
					...
					if($_POST)
					{
						...
						else
						{
							...
							$usid        = $user_token->userid;
							$user_detail     = $this->dynamic_model->get_user_by_id(226);
							...
							if($user_detail){
								...
								$send_data = array('title' => $subject, 'message' => $description, 'token' => $device_token, 'notification_setting' => $notification_setting, 'user_id'=>$usid);
									...
									if (!empty($user_detail['Device_Token']) && strtolower($user_detail['Device_Type']) == 'android') {
										$st = androidPush($send_data);
									}
									if (!empty($user_detail['Device_Token']) && strtolower($user_detail['Device_Type']) == 'ios') {
						...
						else
						{
							...
							if($sender_id == $receiver_id)
							{
								...
								if($receiver)
								{
									...
									if (!empty($receiver['Device_Token']) && strtolower($receiver['Device_Type']) == 'android') {
										$st = androidPush($send_data);
									}
									if (!empty($receiver['Device_Token']) && strtolower($receiver['Device_Type']) == 'ios') {
										$st = iosPush($send_data);
									}
        ...
        else
        {
			...
			else
			{
				...
				else
				{
					...
					$users = $this->dynamic_model->get_all_users('users', $user_id, '', ['Password', 'Transaction_Password']);
	...
	// 		}
	// 		$arg = array();
	// 		$this->form_validation->set_rules('payment_intent_id', '', 'required');
	// 		$this->form_validation->set_rules('payment_method_id', '','required');
	...
	// 			$get_payment = $this->dynamic_model->get_row('payments',array('payment_intent'=> $paymentIntentId));
	// 			if($get_payment['status'] == 'succeeded')
	// 			{
	// 				$arg['status']  = 0;
	// 				$arg['error_code']   = ERROR_FAILED_CODE;
	// 				$arg['error_line']= __line__;
	// 				$arg['message'] =  'Payment already succeeded';
	// 			}
	// 			else{
	// 				\Stripe\Stripe::setApiKey($this->config->item('stripe')['secret_key']);
	// 				try {
	...
	// 					$user_id = getuserid();
	// 					$returnUrl = 'https://kashkash.net/webservices/Users/success';
	...
	// 						);
	// 						$where     = array("payment_intent" => $paymentIntentId);
	// 						$keyUpdate = $this->dynamic_model->updateRowWhere("payments",$where,$payment_data);
							
	// 						$get_payment = $this->dynamic_model->get_row('payments',array('payment_intent'=> $paymentIntentId));
	// 						$receiver_id = $get_payment['receiver_id'];
	// 						$receiver = $this->dynamic_model->get_user_by_id($receiver_id);
	// 						$sender = $this->dynamic_model->get_user_by_id($user_id);
	// 						$sender_balance = $sender['Current_Wallet_Balance'] - ($paymentIntent->amount / 100);
	// 						$receiver_balance = $receiver['Current_Wallet_Balance'] + ($paymentIntent->amount / 100);
							
	// 						$sender_data = array(
	// 							'Current_Wallet_Balance'  => $sender_balance
	// 						);
	// 						$receiver_data = array(
	// 							'Current_Wallet_Balance'  => $receiver_balance
	// 						);
	// 						$whereSender     = array("Id" => $user_id);
	// 						$whereReceiver     = array("Id" => $receiver_id);
	...
	// 								'Tran_Status_Id' => 6,
	// 								'Third_Party_Tran_Id' => $paymentIntent->id,
	// 								'Created_By' => $user_id,
	// 							);
	// 							$new_sender_balance = $current_wallet_balance - $paymentIntent->amount;
	...
	// 							$receiver = $this->dynamic_model->get_user_by_id($receiver_id);
	// 							$current_wallet_balance = $receiver['Current_Wallet_Balance'];
	// 							$new_receiver_balance = $current_wallet_balance + ($paymentIntent->amount / 100);
	...
	// 							$arg['error_code']   = SUCCESS_CODE;
	// 							$arg['error_line']= __line__;
	// 							$arg['message'] = 'Amount Added success';
	// 							}
	// 						else{
	// 							$senderUpdate = $this->dynamic_model->updateRowWhere("users",$whereSender,$sender_data);
	// 							$receiverUpdate = $this->dynamic_model->updateRowWhere("users",$whereReceiver,$receiver_data);
	// 						}
	// 					}
	...
	// 						$arg['error_code']   = SUCCESS_CODE;
	// 						$arg['error_line']= __line__;
	// 						$arg['message'] = 'Payment success';
				
	// 					} elseif ($paymentIntent->status == 'requires_action' && $paymentIntent->next_action->type == 'redirect_to_url') {
	// 						$arg['status']   = 'requires_action';
	// 						$arg['error_code']   = ERROR_FAILED_CODE;
	// 						$arg['error_line']= __line__;
	// 						$arg['paymentIntent'] = $paymentIntent;
	// 						$arg['redirect_to_url'] = $paymentIntent->next_action->redirect_to_url->url;
	...
	// 						$arg['error_code']   = ERROR_FAILED_CODE;
	// 						$arg['error_line']= __line__;
	// 						$arg['message'] = 'No status found. Please try with different payment intent';
	// 						$arg['paymentIntent']= null;
	// 					}
	// 				} catch (Exception $e) {
	// 					$arg['status']   = 0;
	// 					$arg['error_code']   = ERROR_FAILED_CODE;
				...
				if($get_payment['status'] == 'succeeded')
				{
					...
					try {
						...
						if($paymentIntent->status == 'succeeded' ){
							...
							log_message('debug', 'Sender_id - data: ' . $user_id); // Log exception
							log_message('debug', 'receiver_id - data: ' . $receiver_id); // Log POST data
							if ($user_id == $receiver_id) {
								$trandata = array(
									'Tran_Type_Id' => 3,
									'To_User_Id' => $user_id,
								...
								$new_sender_balance = $sender['Current_Wallet_Balance'] - $paymentIntent->amount;

								$receiver = $this->dynamic_model->get_user_by_id($receiver_id);
								$current_wallet_balance = $receiver['Current_Wallet_Balance'];
								
								log_message('debug', 'Current_Wallet_Balance - data: ' . $receiver['Current_Wallet_Balance']);

								$new_receiver_balance = $current_wallet_balance + ($paymentIntent->amount / 100);
		
								log_message('debug', 'New_Wallet_Balance - data: ' . $new_receiver_balance);
								...
								log_message('debug', 'Sender Receiver Equal - data: ' );

								$subject = 'Cash added successfully';
								$token = $sender['Device_Token'];
								$notification_setting = null;
								$message = 'Dear '.$sender['FullName'].',You have successfully added cash of amount '.CURRENCY_SYMBOLE.''.number_format($paymentIntent->amount/100, 2);
								// $type = $receiver['Device_Type'];

								$type = 'android';
								$send_data = array('title' => $subject, 'message' => $message, 'token' => $token, 'notification_setting' => $notification_setting,'user_id' => $user_id);
							
								// $st = self::androidPush($send_data);
								if (!empty($sender['Device_Token']) && strtolower($sender['Device_Type']) == 'android') {
									$st = androidPush($send_data);
								}
								if (!empty($sender['Device_Token']) && strtolower($sender['Device_Type']) == 'ios') {
									$st = iosPush($send_data);
								}
								...
								log_message('debug', 'Sender Receiver Not Equal - data: ' );
								$senderUpdate = $this->dynamic_model->updateRowWhere("users",$whereSender,$sender_data);
								$receiverUpdate = $this->dynamic_model->updateRowWhere("users",$whereReceiver,$receiver_data);
								...
								$token = $receiver['Device_Token'];
								$notification_setting = null;
								$message = 'Dear '.$receiver['FullName'].',You have successfully received amount '.CURRENCY_SYMBOLE.''.number_format($paymentIntent->amount/100, 2). ' from '.$sender['FullName'];
								// $type = $receiver['Device_Type'];
		...
		if (@$user_status['status'] != 0) {
			...
			if ($version_result['status'] != 1) {
				...
				if ($result != 'true') {
					...
					$user_id   = getuserid();
					$loguser = $this->dynamic_model->get_user_by_id($user_id);
					$current_wallet_balance = $loguser['Current_Wallet_Balance'];
					if($amount < $current_wallet_balance)
					{
						try {
							$stripe = new \Stripe\StripeClient($this->config->item('stripe')['secret_key']);
							...
							$updatebalancedata = $this->dynamic_model->updatedata('users', $userbalancedata, $user_id);

							$arg['status'] = 1;
							$arg['error_code'] = SUCCESS_CODE;
							$arg['error_line'] = __line__;
							$arg['transfer_id'] = $transfer->id;
							$arg['message'] =  'Amount transferred succesfully.';
						} catch (Exception $e) {
							$arg['status']  = 0;
							$arg['error_code']   = ERROR_FAILED_CODE;
							$arg['error_line'] = __line__;
				...
				else
				{
					...
					if($_POST)
					{
						...
						else
						{
			        		$account_holder_name = $this->input->post('account_holder_name');
				            $account_holder_type = $this->input->post('account_holder_type');
				            $routing_number 	 = $this->input->post('account_number');
				            $account_number      = $this->input->post('account_number');
				            $phone_number      = $this->input->post('phone_number');
				            $email      	 = $this->input->post('email');
				            $country     	 = $this->input->post('country');
        ...
        else
        {
			...
			else
			{
				...
				else
				{
					...
					$recipients = $this->dynamic_model->getdatafromtable('recipients', $where);
					if($recipients == false)
					{
						$recipients = array();
					}
	...
	// 				$bank_list = $this->dynamic_model->getdatafromtable('user_payment_methods', $where);
					
	// 				$arg['status']  = 1;
	// 				$arg['error_code']   = SUCCESS_CODE;
	// 				$arg['error_line']= __line__;
	// 				$arg['data']= $bank_list;
	// 				$arg['message']= 'User Bank List';
	// 			}
	// 		}
	// 	}
	// 	echo json_encode($arg);
